...
class Samsung {
    ...
    async websocketCmd(aCmd, resolveSent, device) {
        ...
        return new Promise((resolve, reject) => {
            let url = this.getWsUri() + 'channels/samsung.remote.control?' +
            ...
            ws.on('message', function (data) {
                ...
                if (err.code && err.code === 'ECONNREFUSED') {
                    reject('Connection to TV is refused (' + self.getWsUri() + ')');
                } else if (err.code && err.code === 'EHOSTUNREACH') {
                    reject('Connection to TV is unreachable (' + self.getWsUri() + ')');
    ...
    getWsUri() {
        return 'wss://' + this._config.ip_address + ':8002/api/v2/';
    }
