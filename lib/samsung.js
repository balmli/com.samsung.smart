...
module.exports = class Samsung extends SamsungBase {
    ...
    async getStToken() {
        ...
        if (!settings.smartthings) {
            throw new Error('The SmartThings API is not enabled in Adv. settings');
        }
        if (!settings.smartthings_token) {
            throw new Error('The SmartThings API token is not set in Adv. settings');
        }
    ...
    async stDevices() {
        ...
        console.log('stDevices error', response.response.statusCode, response.response.statusMessage);
        if (response.response.statusCode === 401) {
            throw new Error('The API token must be set correctly');
        }
        throw new Error(`Unable to fetch devices (${response.response.statusCode} - ${response.response.statusMessage})`);
    }
    ...
    async getStTvDevice() {
        ...
        if (tvDevices.length === 0) {
            throw new Error('Found no TVs with the SmartThings API.');
        }
    ...
    async getStHealth() {
        try {
            ...
            //console.log('getStHealth', response.data, response.response.statusCode, response.response.statusMessage);
    ...
    async getStInputSources() {
        try {
            ...
            //console.log('getStInputSources', response.data, response.response.statusCode, response.response.statusMessage);
            return response.data.components.main.mediaInputSource.supportedInputSources.value;
        } catch (err) {
            console.log('getStInputSources failed', err);
            return [];
        }
    }
    ...
    async setInputSource(input_source) {
        ...
        if (response.response.statusCode !== 200) {
            console.log('Changing input source failed', response.data, response.response.statusCode, response.response.statusMessage);
            throw new Error('Changing input source failed');
        }
    }
