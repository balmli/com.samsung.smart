...
module.exports = class BaseDevice extends Homey.Device {
    ...
    pollMethods(timeout) {
        return [
            ...
            { id: 'st', method: this.isSmartThingsEnabled() ? this._samsung.getStHealth() : undefined }
        ];
    }
    ...
    async isDeviceOnline(timeout) {
        ...
        const onOff = pollResults.reduce((acc, val) => acc = acc || val);
    ...
    async pollDevice() {
        ...
        const onOff = await this.isDeviceOnline();
    ...
    async turnOnOff(onOff) {
        ...
        let isOnOff = await this.isDeviceOnline(500);
        if (onOff === isOnOff) {
            ...
            if (this._upnpClient) {
                ...
                if (this.hasCapability('volume_set')) {
                    ...
                    if (volumeState !== undefined) {
                        ...
                        if (volume !== this.getCapabilityValue('volume_set')) {
