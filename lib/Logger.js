 ...
 */
class Logger {
	constructor({ homey, logLevel = 4, captureLevel = 5, prefix, logFunc = console.log, errorFunc = console.error}, homeyEnv) {

		// Load homey-log and create pre-bound functions or function stubs for logger
		const homeyLog = typeof homeyEnv.HOMEY_LOG_URL === 'string' ? new Log({ homey }) : null; // eslint-disable-line
