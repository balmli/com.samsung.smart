...
module.exports = class SamsungBase {
    ...
    async wake() {
        const self = this;
        let mac = this._config.mac_address;
        if (!mac) {
            mac = await this._config.homey.arp.getMAC(this._config.ip_address);
            if (!mac || !mac.indexOf(':') >= 0) {
                throw new Error(this.i18n.__('errors.missing_mac_address'));
            }
        }
    ...
    _delay(ms) {
        return new Promise(resolve => {
            this._config.homey.setTimeout(resolve, ms);
        });
    }
